...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof initializePostFunctionality === "function") {
    ...
    console.error(
      "initializePostFunctionality function not available. Make sure post.js is loaded."
    );
  }
});
  ...
  reader.onload = (e) => {
    if (type === "cover") {
      ...
      const placeholder = document.querySelector(
        "#coverUploadArea .upload-placeholder"
      );
      ...
      const placeholder = document.querySelector(
        "#profileUploadArea .upload-placeholder"
      );
      if (preview && previewImg && placeholder) {
        previewImg.src = e.target.result;
        preview.style.display = "block";
  ...
  fetch(profileUrl, {
    ...
    .then((text) => {
      ...
      if (data.success) {
        ...
        // Close modal
        const modal = window.bootstrap.Modal.getInstance(
          document.getElementById("coverPhotoModal")
        );
        if (modal) {
          modal.hide();
        }
        ...
        showNotification(
          "Error uploading cover photo: " + data.message,
          "error"
        );
      }
    })
    ...
    .then((text) => {
      ...
      if (data.success) {
        ...
        const profileImages = document.querySelectorAll(
          "#profilePic, .profile-pic-nav, .user-dropdown-avatar"
        );
        ...
        showNotification(
          "Error uploading profile photo: " + data.message,
          "error"
        );
      }
    })
...
async function checkFriendshipStatus() {
  ...
  try {
    const response = await fetch(
      `${window.ROOT}/friendship/get_friendship_status/${profileUserId}`
    );
    ...
    switch (currentStatus) {
      case "Add Friend":
        response = await fetch(
          `${window.ROOT}/friendship/send_request/${profileUserId}`,
          {
            method: "POST",
          }
        );
      ...
      case "Accept Request":
        response = await fetch(
          `${window.ROOT}/friendship/accept_request/${profileUserId}`,
          {
            method: "POST",
          }
        );
      ...
      case "Friends":
        if (confirm("Are you sure you want to unfriend this user?")) {
          response = await fetch(
            `${window.ROOT}/friendship/unfriend/${profileUserId}`,
            {
              method: "POST",
            }
          );
...
async function loadFriendsGrid() {
  ...
  try {
    const response = await fetch(
      `${window.ROOT}/friendship/get_friends/${profileUserId}`
    );
    ...
    if (!data.friends || data.friends.length === 0) {
      ...
      friendsGrid.innerHTML = displayFriends
        .map(
          (friend) => `
        <div class="friend-item" onclick="window.location.href='${
          window.ROOT
        }/profile/${friend.friend_id}'">
          <img src="${
            friend.profile_pic || "${window.ROOT}/assets/images/default_pfp.png"
          }" class="friend-avatar" alt="${friend.friend_name}">
    ...
    searchInput.addEventListener("input", (e) => {
      ...
      friendItems.forEach((item) => {
        const friendName = item
          .querySelector(".friends-modal-name")
  ...
  try {
    ...
    if (!data.friends || data.friends.length === 0) {
      friendsModalList.innerHTML =
        '<div class="text-center text-muted">No friends yet</div>';
    } else {
      const friends = data.friends;
      // Ensure friends is an array
      const friendsArray = Array.isArray(friends)
        ...
        ? [friends]
        : [];
      friendsModalList.innerHTML = friendsArray
        .map(
          (friend) => `
        <div class="friends-modal-item" onclick="window.location.href='${
          window.ROOT
        }/profile/${friend.friend_id}'">
          <img src="${
            friend.profile_pic || "/Friendora/assets/images/default_pfp.png"
          }" class="friends-modal-avatar" alt="${friend.friend_name}">
          <div class="friends-modal-info">
            <h6 class="friends-modal-name">${friend.friend_name}</h6>
            <p class="friends-modal-meta">${
    ...
    console.error("Failed to load friends:", error);
    friendsModalList.innerHTML =
      '<div class="text-center text-muted">Failed to load friends</div>';
  }
}
...
function openPhotoGallery() {
  const modal = new window.bootstrap.Modal(
    document.getElementById("photoGalleryModal")
  );
  loadPhotoGallery();
  modal.show();
}
...
console.log(
  "🌟 Friendora Profile Page Loaded - Welcome to the Digital Dreamscape! 🌟"
);
