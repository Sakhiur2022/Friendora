...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(() => {
        ...
        console.log("Reaction buttons found:", reactionBtns.length)
        ...
        if (commentBtns.length > 0) {
          console.log("Testing comment button click on first post...")
          setTimeout(() => {
            commentBtns[0].click()
            console.log("Clicked first comment button")
          }, 500)
        }
        
        // Test all reaction icons
        console.log("Testing all reaction icons:")
        ...
        console.log("Desktop notification count element:", !!desktopNotifCount)
        console.log("Mobile notification count element:", !!mobileNotifCount)
        console.log("Mobile message count element:", !!mobileMessageCount)
    ...
    setTimeout(() => {
        ...
        showNotification('Welcome to Friendora! 🎵 Posts and reactions are ready to use', 'success');
    }, 1000);
  })
  ...
  const allPhotosData = [
    "https://via.placeholder.com/300x300/6f42c1/ffffff?text=VR+Art+1",
    "https://via.placeholder.com/300x300/dc3545/ffffff?text=Neo+Tokyo+1",
    "https://via.placeholder.com/300x300/28a745/ffffff?text=Digital+Art+1",
    "https://via.placeholder.com/300x300/ffc107/000000?text=Cyber+City+1",
    "https://via.placeholder.com/300x300/007bff/ffffff?text=VR+Art+2",
    "https://via.placeholder.com/300x300/17a2b8/ffffff?text=Neo+Tokyo+2",
    "https://via.placeholder.com/300x300/6c757d/ffffff?text=Digital+Art+2",
    "https://via.placeholder.com/300x300/fd7e14/ffffff?text=Cyber+City+2",
    "https://via.placeholder.com/300x300/e83e8c/ffffff?text=VR+Art+3",
    "https://via.placeholder.com/300x300/20c997/ffffff?text=Neo+Tokyo+3",
    "https://via.placeholder.com/300x300/6f42c1/ffffff?text=Digital+Art+3",
    "https://via.placeholder.com/300x300/dc3545/ffffff?text=Cyber+City+3",
  ]
  ...
  function loadDummyData() {
    ...
    setTimeout(() => {
      ...
      if (!postsContainer || postsContainer.children.length === 0) {
        console.log("Posts not loaded properly, retrying...")
        loadEnhancedPosts()
      }
    }, 1000)
  ...
  function loadEnhancedPosts() {
    console.log("Loading enhanced posts...")
    ...
    if (!postsContainer) {
      console.log("Posts container not found, trying alternative selectors...")
      ...
      if (!altContainer) {
        console.error("CRITICAL: Cannot find posts container! Page may not be loaded correctly.")
        ...
        setTimeout(() => {
          showNotification('Error: Posts container not found! Please refresh the page.', 'error')
        }, 1000)
        return
      }
    }
    
    console.log("Posts container found, loading", postsData.length, "posts")
    console.log("Container innerHTML before:", postsContainer.innerHTML.length, "characters")
                  ...
                  <div class="post-actions">
                      <div class="reaction-container">
                          <button class="post-action-btn reaction-btn ${post.userReaction ? 'reacted' : ''}" 
                              ...
                              <span class="reaction-label">${post.userReaction ? capitalizeFirst(post.userReaction) : 'Like'}</span>
                          ...
                          <div class="reaction-summary" onclick="showReactionDetails(${post.id})">
    ...
    console.log("Container innerHTML after:", postsContainer.innerHTML.length, "characters")
  
    // Add stagger animation to posts
    const posts = postsContainer.querySelectorAll(".post-card")
    console.log("Found", posts.length, "post cards after loading HTML")
    ...
    console.log("Found", reactionBtns.length, "reaction buttons")
    
    // Verify comment buttons are created
    const commentBtns = postsContainer.querySelectorAll(".comment-btn")
    console.log("Found", commentBtns.length, "comment buttons")
    ...
    console.log("Found", reactionEmojis.length, "reaction emojis")
    
    // Log first emoji content for debugging
    if (reactionEmojis.length > 0) {
      console.log("First emoji content:", reactionEmojis[0].textContent)
    }
    
    console.log("Enhanced posts loaded successfully!")
    
    // Show success notification
    setTimeout(() => {
      showNotification(`Successfully loaded ${posts.length} posts with reactions and comments!`, 'success')
    }, 500)
  }
  ...
  function loadNotifications() {
    ...
    console.log("Desktop notifications list found:", !!notificationsList)
    console.log("Desktop notification count found:", !!notificationCount)
    console.log("Mobile notification count found:", !!mobileNotificationCount)
    console.log("Mobile notifications list found:", !!mobileNotificationsList)
    ...
    if (notificationCount) {
      ...
      console.log("Desktop notification count updated to:", unreadCount)
    }
  ...
  function updateMessageCount() {
    ...
    console.log("Desktop messages icon found:", !!messagesIcon)
    if (messagesIcon) {
      ...
      console.log("Desktop message count updated to:", messageCount)
    }
    
    // Update mobile message count
    const mobileMessageCount = document.getElementById("mobileMessageCount")
    console.log("Mobile message count element found:", !!mobileMessageCount)
    if (mobileMessageCount) {
      ...
      console.log("Mobile message count updated to:", messageCount)
    }
  }
  ...
  function getReactionIcon(reaction) {
    ...
    console.log(`Getting icon for reaction: ${reaction} -> ${icons[reaction] || "👍"}`)
  ...
  function quickReaction(postId) {
    console.log(`Quick reaction for post ${postId}`)
  ...
  function showReactionDetails(postId) {
    ...
    showNotification(`Reaction Details:\n${reactionBreakdown}`, 'info')
  }
  ...
  function showReactions(postId) {
    ...
    showNotification(`This post has ${totalReactions} reaction${totalReactions !== 1 ? 's' : ''}`, 'info')
  }
  
  function addReaction(postId, reactionType) {
    console.log(`Adding reaction: ${reactionType} to post ${postId}`)
    ...
    showNotification(`${actionText} ${emoji} ${reactionType} reaction!`, 'success')
  ...
  function updateReactionUI(postId, post) {
    ...
    console.log("Found reaction button:", reactionBtn)
    
    if (!reactionBtn) {
      console.log("Could not find reaction button")
      return
    }
    
    const emoji = reactionBtn.querySelector(".reaction-emoji")
    const label = reactionBtn.querySelector(".reaction-label")
    const count = reactionBtn.querySelector(".reaction-count")
    const summary = reactionBtn.parentElement.querySelector(".reaction-summary")
    
    console.log("Found elements - emoji:", !!emoji, "label:", !!label, "count:", !!count)

    if (emoji && label && count) {
      ...
      const newLabel = post.userReaction ? capitalizeFirst(post.userReaction) : "Like"
      
      console.log("Setting emoji to:", newEmoji)
      console.log("Setting color to:", newColor)
      console.log("Setting label to:", newLabel)
      console.log("Setting count to:", post.totalReactions)
      ...
      console.log("Could not find required UI elements")
    }
  }
  ...
  function toggleComments(postId) {
    ...
    if (!postCard) {
      console.log("Post card not found, posts may not be loaded yet. Attempting to reload posts...")
      loadEnhancedPosts()
      ...
      setTimeout(() => {
        ...
        if (retryPostCard) {
          console.log("Post found after reload, retrying comment toggle...")
          toggleComments(postId)
        } else {
          showNotification('Error: Post not found. Please refresh the page.', 'error')
        }
      }, 1000)
      return
    }
    ...
    if (!commentsSection) {
      console.log("Comments section not found:", `commentsSection${postId}`)
      // Try to find it within the post card
      const altCommentsSection = postCard.querySelector('.comments-section')
      console.log("Alternative comments section found:", !!altCommentsSection)
      
      if (!altCommentsSection) {
        console.error("CRITICAL: Comments section not found even within post card!")
        showNotification('Error: Comments section not found. Please refresh the page.', 'error')
        return
      } else {
        // Use the alternative comments section
        console.log("Using alternative comments section")
    ...
    console.log("Comments currently visible:", isVisible)
    ...
    if (isVisible) {
      ...
      setTimeout(() => {
        ...
        if (commentInput) {
          ...
          console.log("Comment input not found:", `commentInput${postId}`)
        }
      }, 350)
    }
    ...
    if (commentBtn) {
      ...
      console.log("Updated comment button appearance")
    } else {
      console.log("Comment button not found within post card")
    }
...
function loadAboutSection() {
    ...
    const aboutItems = [
        { icon: "bi-geo-alt", label: "Lives in", value: userData.currentCity },
        { icon: "bi-house", label: "From", value: userData.hometown },
        { icon: "bi-calendar", label: "Born", value: new Date(userData.birthday).toLocaleDateString() },
        { icon: "bi-person", label: "Gender", value: userData.gender },
        { icon: "bi-building", label: "Works at", value: userData.workplace },
        { icon: "bi-mortarboard", label: "Studied at", value: userData.university },
        { icon: "bi-globe", label: "Website", value: userData.website },
        { icon: "bi-chat", label: "Languages", value: userData.language }
    ];

    aboutSection.innerHTML = aboutItems.map(item => `
        <div class="about-item">
            <i class="${item.icon}"></i>
...
function openPhotoModal(photoUrl, index) {
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content cyber-modal">
                    ...
                    <div class="modal-body text-center">
                        <img src="${photoUrl}" class="img-fluid" alt="Photo">
                    </div>
                </div>
            </div>
        </div>
    ...
    const photoModal = new bootstrap.Modal(modal.querySelector('#photoModal'));
    photoModal.show();
    
    modal.querySelector('#photoModal').addEventListener('hidden.bs.modal', () => {
        modal.remove();
    });
}
...
function populateEditForm() {
    const fields = [
        'currentCity', 'hometown', 'birthday', 'gender', 'highSchool', 
        'college', 'university', 'workplace', 'politicalViews', 'religion',
        'country', 'website', 'language', 'professional', 'quotes', 'lifeEvents'
    ];
