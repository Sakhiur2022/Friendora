              ...
              <div id="imagePreviewContainer" class="mt-3" style="display: none;">
                <div class="position-relative d-inline-block">
                  <img id="imagePreview" src="${
                    window.ROOT || ""
                  }/assets/images/placeholder.svg" alt="Preview" class="img-fluid rounded" style="max-height: 200px;">
...
function initializeCreatePostForm(isOwnProfile, userAvatar, userName) {
  ...
  if (createPostContainer) {
    createPostContainer.innerHTML = generateCreatePostHTML(
      userAvatar,
      userName
    );
  }
...
function loadUserPosts(userId) {
  ...
  postsContainer.innerHTML =
    '<div class="text-center"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading posts...</span></div></div>';

  const postUrl = window.ROOT
    ? `${window.ROOT}/post/getUserPosts/${userId}`
...
function createPostHTML(post) {
  ...
  return `
    ...
    post.author_name
  }" class="post-author-pic me-3">
          <div class="flex-grow-1">
            <h6 class="mb-0 post-author-name">${createUserLink(
              post.creator_id,
              post.author_name
            )}</h6>
        ...
        <div class="post-actions cyber-actions-grid">
          <div class="reaction-selector">
            <button class="btn cyber-btn-ghost main-reaction-btn ${
              post.user_reaction ? "reacted" : ""
            }" 
                    data-reaction="${
                      post.user_reaction || "like"
                    }" onclick="toggleReactionSelector(${post.id})">
              <i class="bi ${getReactionIcon(
                post.user_reaction || "like"
              )} me-2 reaction-icon"></i>
              <span class="reaction-text">${getReactionText(
                post.user_reaction || "like"
              )}</span>
            </button>
            <div class="reaction-dropdown cyber-dropdown" id="reactionDropdown${
              post.id
            }">
              <button class="reaction-option" onclick="reactToPost(${
                post.id
              }, 'like')">
                <i class="bi bi-heart-fill cyber-heart"></i>
                <span>Like</span>
          ...
          <button class="btn cyber-btn-ghost comment-btn" onclick="toggleComments(${
            post.id
          })">
          ...
          <button class="btn cyber-btn-ghost share-btn" onclick="sharePost(${
            post.id
          })">
...
function displayComments(postId, comments) {
  ...
  commentsList.innerHTML = comments
    ...
    <div class="comment-item d-flex align-items-start mb-2" data-comment-id="${
      ...
      }" class="comment-avatar me-2" alt="${comment.user_name || "User"}">
      <div class="comment-content flex-grow-1">
        <div class="comment-bubble">
          <strong class="comment-author">${createUserLink(
...
function sharePost(postId) {
  ...
  fetch(postUrl, {
    ...
    .then((text) => {
      ...
      if (data.success) {
        showNotification(
          "✨ Post shared in the digital dreamscape! ✨",
          "success"
        );
        ...
        showNotification(
          "Unable to share in the dreamscape: " + data.message,
          "error"
        );
      }
    })
    ...
    .then((text) => {
      ...
      if (data.success) {
        ...
        const reactionBtn = document.querySelector(
          `[data-post-id="${postId}"] .main-reaction-btn`
        );
        if (reactionBtn) {
          if (data.action === "removed") {
            reactionBtn.classList.remove("reacted");
            reactionBtn.setAttribute("data-reaction", "like");
            reactionBtn.innerHTML = `<i class="bi ${getReactionIcon(
              "like"
            )} me-2 reaction-icon"></i><span class="reaction-text">${getReactionText(
              "like"
            )}</span>`;
            ...
            showNotification(
              `${reactionEmojis[reactionType]} Emotion resonates through the dreamscape!`,
              "success"
            );
...
function updatePostReactionCounts(postId, counts) {
  ...
  reactionTypes.forEach((type) => {
    const countElement = document.querySelector(
      `[data-post-id="${postId}"] .reaction-count[data-type="${type}"] .count-number`
    );
  ...
  if (counts.share_count !== undefined) {
    const shareCountElement = document.querySelector(
      `[data-post-id="${postId}"] .share-count .count-number`
    );
...
function loadPostByIds(userIds) {
  console.log("loadPostByIds() called for userIds:", userIds);
  ...
  const postUrl = window.ROOT
    ? `${window.ROOT}/post/getPostsByUsers`
...
// Setup comment functionalities for posts
function setupCommentFunctionalities(posts) {
  posts.forEach((post) => {
    const postElement = document.querySelector(`[data-post-id="${post.id}"]`);
    if (postElement) {
      ...
      // Load existing comments if comments section is visible
